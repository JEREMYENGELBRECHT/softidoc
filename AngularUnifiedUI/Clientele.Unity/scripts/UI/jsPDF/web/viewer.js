...
'use strict';
var PDF_ALLOW_MULTIPLE_ALLOCATION = false;
var PDF_SELECT_THUMBNAIL = false;
...
var PDFView = {
    ...
    open: function pdfViewOpen(url, accesstoken, scale, password,
        ...
        if (accesstoken != "") {
            ...
            parameters.httpHeaders.Authorization = 'Bearer ' + accesstoken;
        ...
        firstPagePromise.then(function (pdfPage) {
            ...
            for (var pageNum = 1; pageNum <= pagesCount; ++pageNum) {
                ...
                var thumbnailView = new ThumbnailView(thumbsView, pageNum,
                                                      viewportClone, PDF_SELECT_THUMBNAIL);
    ...
    rotatePages: function pdfViewPageRotation(delta) {
        ...
        parent.SendIFrameData("OnJsPdfViewerRotate", this.pageRotation);
...
document.addEventListener('DOMContentLoaded', function webViewerLoad(evt) {
    ...
    var accesstoken = params.token || DEFAULT_URL;
    PDF_SELECT_THUMBNAIL = params.selectThumbnails || false;
    PDF_SELECT_THUMBNAIL = eval(PDF_SELECT_THUMBNAIL);

    PDF_ALLOW_MULTIPLE_ALLOCATION = params.allowMultipleAllocation || false;
    PDF_ALLOW_MULTIPLE_ALLOCATION = eval(PDF_ALLOW_MULTIPLE_ALLOCATION);
    ...
    //#if !B2G
    PDFView.open(file, accesstoken, 0);
    //#endif
}, true);
...
function ResetChecboxes() {
    ...
    for (var i = 0; i < myCheckboxList.length; i++) {
        if (allowMultipleAllocation == false && (myCheckboxList[i].checked)) {
            var item = document.getElementById(myCheckboxList[i].parentNode.id);
            item.style.display = "none";
            continue;
        }
        myCheckboxList[i].checked = false;
    }
}
