var TC_CONF = TC_CONF || {
    // TC_CONF_SECTION
    "firstPartyCookieBaseDomainEnabled": true, "maxQueueCookieSize": 4093, "activityLevel": "FULL", "thirdPartyCookie": false, "trackingTime": 63072000000
...
var TC = TC || (function () {
    ...
    var TCT = {
        ...
        collector: function () {
            return "us-east-1-capture.lift.acquia.com/tc";
        },

        generator: function () {
            return "us-east-1-capture.lift.acquia.com/tc3ptidgen";
        },

        doNotTrackController: function () {
            return "us-east-1-capture.lift.acquia.com/tc3dnt";
        }
    ...
    var Lang = {
        ...
        "topLevelDomain": function (host) {
            ...
            if (host.split('.').length === 1) {
                ...
                for (i = domainParts.length - 1; i >= 0; i--) {
                    domain = domainParts.slice(i).join('.');
                    document.cookie = 'tc_tld_test=true;path=/;domain=.' + domain + ';';
                    if (document.cookie.indexOf("tc_tld_test=true") > -1) {
                        document.cookie = 'tc_tld_test=;path=/;domain=.' + domain + ';max-age=0'
                        return domain;
                    }
                }
            }
    ...
    function Cookie(name) {
        ...
        var useSubDomain = Lang.isDefined(TC_CONF) && Lang.isDefined(TC_CONF.firstPartyCookieBaseDomainEnabled) && (TC_CONF.firstPartyCookieBaseDomainEnabled == "false" || TC_CONF.firstPartyCookieBaseDomainEnabled == false);
        ...
        if (useSubDomain) {
            ...
            this.domain = Lang.topLevelDomain(domain);
        }
    ...
    //
    function Config() { };
    Config.setAccount = function (account, customerSite) {
        Config.account = account;
        Config.customerSite = customerSite;
    ...
    Tracker._track = function (params, appendedParams) {
        ...
        if (!Lang.isUndefined(Config.customerSite)) {
            params.tcvs = Config.customerSite;
        }
        ...
        params.tcectz = new Date().getTimezoneOffset();
    ...
    Tracker._sendTrack = function (queryString) {
        // If different touch id, set flag to indicate that it's an old touch
        if (queryString.indexOf('tcttid=' + encodeURIComponent(SessionID.value())) == -1) {
            queryString = queryString + "&tcttidold=" + encodeURIComponent("true");
        }
    ...
    var API = {
        ...
        getUniversalID: function (callback) {
            var id = UniversalID.value();
            //allocates a UniversalID if it does not exist.
            if (id === null) {
                UniversalID.set();
            }
            callback(id);
        ...
        if (Config.isActivityLevelCapture()) {
            ...
            if (BOM.isSupported() && Config.isActivityLevelFull()) {
                ...
                Config.setListener(function (params) {
                    _tcwq.unshift(['init',
						 { 'tcptid': UniversalID.value(), 'tcttid': SessionID.value(), 'tclv': TCT.version(), 'tcvs': Config.customerSite, 'tcla': Config.account, 'tcep': BOM.getPlatform(), 'tcnu': UniversalID.isNewUser() }]);
                    // Load the widget tag here
                    var s = document.createElement('script');
                    s.type = 'text/javascript';
                    s.async = true;
                    s.src = ('https:' == document.location.protocol ? 'https' : 'http') + '://' + 'cdn.lift.acquia.com/' + Config.account + '/' + jsFileToLoad;
                    var x = document.getElementsByTagName('script')[0];
